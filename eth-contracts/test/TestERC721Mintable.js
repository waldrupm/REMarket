...
contract('ERC721MintableComplete', accounts => {
    ...
    describe('Ownable inherited', function () {
        ...
        it('should NOT allow unauthorized address to transfer ownership', async() => {
            await expectToRevert(contractInstance.transferOwnership(account_two, {from: account_two}), 'Must be owner to do that');
        });
        ...
        it('should fail when minting when caller is not contract owner', async() => { 
            await expectToRevert(contractInstance.mint(account_two, 12, {from: account_one}), 'Must be owner to do that');
        });
    ...
    describe('TPausable inherited', function () {
        ...
        it('should NOT allow unauthorized address to pause the contract', async() => {
            await expectToRevert(contractInstance.pause({from: account_two}), 'Must be owner to do that');
        });

        it('should NOT allow unauthorized address to unpause the contract', async() => {
            await expectToRevert(contractInstance.unpause({from: account_two}), 'Must be owner to do that');
        });
        ...
        it('should allow owner to pause the contract and emit event', async() => {
            ...
            truffleAssert.eventEmitted(tx, 'Paused', (ev) => {
                return expect(ev.pausedBy).to.deep.equal(account_one);
            });
        });

        it('should NOT allow owner to pause the contract when contract is already paused', async() => {
            await expectToRevert(contractInstance.pause({from: account_one}), 'Contract is paused');
        });
        ...
        it('should allow owner to unpause the contract and emit event', async() => {
            ...
            truffleAssert.eventEmitted(tr, 'Unpaused', (ev) => {
                return expect(ev.unpausedBy).to.deep.equal(account_one);
            });
        });

        it('should NOT allow owner to unpause the contract when contract is already unpaused', async() => {
            await expectToRevert(contractInstance.unpause({from: account_one}), 'Contract is not paused');
        });
    ...
    describe('Test suite: ERC721MintableComplete', () => {
       const tokensIds = [11, 22, 33, 44, 55, 66, 77, 88, 99, 101];
        ...
        it('should not mint an already existent tokenId', async() => {
            await expectToRevert(contractInstance.mint(accounts[8], tokensIds[3]), 'That token exists or the to address is invalid');
        });

        it('should not mint to an invalid address 0x0', async() => {
            await expectToRevert(contractInstance.mint(zeroAddress, 211), "That token exists or the to address is invalid");
        });
        ...
        it('should allow the token owner to approve another user for its token', async() => {
            ...
            truffleAssert.eventEmitted(tx, 'Approval', (ev) => {
                return expect(ev.owner).to.deep.equal(accounts[8]) 
                       && expect(ev.approved).to.deep.equal(accounts[9])
                       && expect(ev.tokenId.toNumber()).to.equal(tokensIds[7]);
            });
        ...
        it('should NOT allow to transfer to an invalid address 0x0', async() => {
            await expectToRevert(contractInstance.transferFrom(accounts[8], zeroAddress, tokensIds[7], {from: accounts[8]}), "To address is not valid");
        });
    });
